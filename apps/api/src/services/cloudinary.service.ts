 ...
 * @constant {Object}
 * @property {number} MAX_FILE_SIZE_BYTES - 업로드 허용 최대 파일 크기 (5MB)
 * @property {string} DEFAULT_FOLDER - 기본 저장 폴더명
 */
...
export class CloudinaryService {
   ...
   *
   * @async
   * @param {File} file - 업로드할 Web Standard File 객체
   * @param {string} [folder=CLOUDINARY_CONFIG.DEFAULT_FOLDER] - Cloudinary 내 저장될 폴더 경로
   * @returns {Promise<UploadApiResponse>} Cloudinary 업로드 응답 결과
   ...
   *
   * @async
   * @param {string} publicId - 삭제할 파일의 Cloudinary 퍼블릭 ID
   * @returns {Promise<{ result: string }>} 삭제 결과 응답 (성공 시 { result: 'ok' })
   ...
   *
   * @param {string} url - Cloudinary 파일 전체 URL
   * @returns {string | null} 추출된 퍼블릭 ID 문자열 (추출 불가 시 null)
   */
